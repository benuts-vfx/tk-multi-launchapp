...
class AppLaunch(tank.Hook):
    ...
    def execute(
        ...
        if engine_name == "tk-alias":
            ...
            python_path = os.environ.get("ALIAS_PLUGIN_SERVER_PYTHON")
            if python_path:
                python_exe = f"{python_path}\\python.exe"
            else:
                python_exe = sys.executable
            app_str = "\"App\""
            alias_path = f"\"{app_path}\""
            cmd = f"import os;os.system(r'start /B {r''+ app_str + r''} {r'' + alias_path + r''} {r'' + app_args + r''}')"
            try:
